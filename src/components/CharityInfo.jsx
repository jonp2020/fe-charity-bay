...
class CharityInfo extends Component {
  ...
  this.setState((currentState) => {
    return {
      charityShowMore: currentState.charityShowMore === "showLess" ? "showMore" : "showLess",
      buttonText: currentState.buttonText === "Show More" ? "Show Less" : "Show More"
  }
  })
}
  render() {
    ...
    return (
         <section >
           <div className="charityCardWrapper">
                <p><strong>{this.props.charity.name}</strong></p>
                <img src={require(`../images/${this.props.charity.charity_id}.png`)} alt={this.props.charity.name}></img>
               <button className="charityDisplay-btn" onClick={this.handleDescription} value="ShowMore">{this.state.buttonText}</button>
               </div>
                <p className={this.state.charityShowMore}>{this.props.charity.description}</p>
              </section>
    );
  }
}
